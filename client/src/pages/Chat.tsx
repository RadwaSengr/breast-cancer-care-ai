...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    if (!saved) return;
    try { setMessages(JSON.parse(saved) as PatientMessage[]); } catch { window.localStorage.removeItem(`aftercare-static-messages-${sessionId}`); }
  }, [sessionId]);

  useEffect(() => {
    if (messages.length) window.localStorage.setItem(`aftercare-static-messages-${sessionId}`, JSON.stringify(messages));
  }, [messages, sessionId]);
  ...
  const send = (message: string) => {
    ...
    window.setTimeout(() => {
      ...
      setMessages(current => [...current, { id: `assistant-${Date.now()}`, role: "assistant", content: response.answer, citations: response.citations, suggestedQuestions: response.suggestedQuestions, feedback: null }]);
      setIsThinking(false);
    }, 180);
  };
  ...
  return (
    <div dir={isArabic ? "rtl" : "ltr"} className={cn("brand-pink min-h-screen bg-[#fff8fb] text-slate-900", isArabic && "font-arabic")}>
      ...
      <main className="container max-w-6xl py-8 sm:py-12">
        ...
        <AIChatBox messages={messages} onSendMessage={send} onFollowUp={send} onFeedback={() => undefined} isLoading={isThinking} disabled={false} language={language} copy={t} />
      </main>
    </div>
  );
}
