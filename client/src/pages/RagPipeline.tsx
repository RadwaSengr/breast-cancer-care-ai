...
export default function RagPipeline() {
  ...
  const selectBundledPdf = async () => {
    try {
      const response = await fetch("/sources/life-after-treatment.pdf");
      if (!response.ok) throw new Error("Unable to load the included NCI PDF.");
      const blob = await response.blob();
      setSelectedFile(new File([blob], "life-after-treatment.pdf", { type: "application/pdf" }));
      setTitle("Facing Forward: Life After Cancer Treatment");
      setOrganization("National Cancer Institute");
      setSourceUrl("https://www.cancer.gov/publications/patient-education/life-after-treatment.pdf");
      setNotice(t.bundledPdfReady);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Unable to load the included NCI PDF.");
    }
  };
    ...
    <div dir={isArabic ? "rtl" : "ltr"} className={cn("brand-pink min-h-screen bg-[#fff8fb] text-slate-900", isArabic && "font-arabic")}>
      ...
      <main className="container max-w-6xl py-8 sm:py-12">
        ...
        <section className="mt-10 grid gap-6 lg:grid-cols-2"><div className="rounded-[1.5rem] border border-slate-100 bg-white p-6 shadow-sm"><p className="eyebrow">{t.scope}</p><div className="mt-5 grid gap-4 sm:grid-cols-2"><div className="rounded-2xl bg-teal-50 p-4"><h2 className="flex items-center gap-2 font-display font-bold text-teal-900"><CheckCircle2 className="size-4" />{t.included}</h2><ul className="mt-3 space-y-2 text-sm leading-6 text-teal-900/80">{(overview.data?.scope?.included ?? [
          isArabic ? "تثقيف ودعم التعافي ورعاية ما بعد علاج سرطان الثدي" : "General breast-cancer after-care education and survivorship support",
          isArabic ? "الاستعداد لموعد الطبيب، والمتابعة والتأهيل والدعم الموثق" : "Appointment preparation, follow-up care, rehabilitation, and support"
        ]).map(item => <li key={item}>• {item}</li>)}</ul></div><div className="rounded-2xl bg-rose-50 p-4"><h2 className="flex items-center gap-2 font-display font-bold text-rose-900"><LockKeyhole className="size-4" />{t.excluded}</h2><ul className="mt-3 space-y-2 text-sm leading-6 text-rose-900/80">{(overview.data?.scope?.excluded ?? [
          isArabic ? "التشخيص الفردي، تحديد خطط العلاج الشخصية، وجرعات الأدوية" : "Diagnosis, individualized prognosis, treatment selection, medication doses",
          isArabic ? "المصادر غير المدققة، والسجلات الشخصية والمواد مجهولة الرابط" : "Unvetted sources, personal medical records, and unverified links"
        ]).map(item => <li key={item}>• {item}</li>)}</ul></div></div></div><div className="rounded-[1.5rem] border border-teal-100 bg-teal-50/65 p-6"><p className="eyebrow">{t.sourceControl}</p><p className="mt-4 text-sm leading-7 text-slate-600">{t.sourceControlBody}</p><div className="mt-5 flex items-center gap-3 rounded-2xl border border-teal-100 bg-white px-4 py-3 text-sm font-semibold text-teal-900"><ShieldCheck className="size-5 text-teal-700" />WHO · NCI · NCCN · ASCO · بهية</div></div></section>
